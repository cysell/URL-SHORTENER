...
<div class="panel panel-default">
  <div class="panel-heading">
    <?php if(!empty($this->id)) echo "User's " ?>网址管理
  </div>      
  <div class="panel-body nopadding">
    <form action="<?php echo Main::ahref("urls/delete")?>" method="post" id="delete-all-urls">
    	<div class="toolbar">
        <div class="btn-group">
          <a href="#check-all" class="btn btn-primary" id="check-all">选择所有</a>
          <input type='submit' class="btn btn-danger" value='反选'>
        </div>  
        ...
        <?php if (!isset($hideFilter)): ?>
          <select name="filter" class="hidden-xs" id="filter" data-search="0">
            <optgroup label="排序">
              ...
              <option value="less"<?php if(Main::is_set('filter','less')) echo " selected" ?>>冷门</option>       
              <option value="anon"<?php if(Main::is_set('filter','anon')) echo " selected" ?>>匿名</option>
            </optgroup>
          </select>          
        <?php endif ?>
    	</div>
        ...
        <div class="url-list">          
          ...
          <div class="url-action">
            ...
            <?php if (!empty($url->location)): ?>
              <i class="glyphicon glyphicon-globe"></i> 地理定位
            <?php endif ?>
            <?php if (!empty($url->pass)): ?>
              <i class="glyphicon glyphicon-lock"></i> 密码网址
            <?php endif ?> 
