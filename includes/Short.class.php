...
class Short extends App{	
    ...
    // If require registration
		...
		if(in_array($ext, array(".zip",".rar",".7z",".flv",".mp4",".avi",".mp3",".jpeg",".png",".jpg",".gif",".mk4",".iso"))){
			$meta_title = "这是一个下载网址.";
			$meta_description = "请注意，此短网址是一个下载文件.";
		
		}elseif($preg_schemes && preg_match('~('.$preg_schemes.'://)(.*)~', $array["url"])){
			$meta_title = "";
			$meta_description = "此URL正在使用自定义协议: {$protocol}";
	...
	private function update($url){
		...
		if(!$this->db->get("stats", ["ip" => Main::ip()], ["limit" => 1])){
	...
	protected function qr(){
		...
		$api_url = "https://bshare.optimix.asia/barCode?site=weixin&url=".urlencode((!empty($url->domain) ? $url->domain : $this->config["url"])."/{$this->action}?source=qr");	
	...
	protected function ico(){
		...
		if($preg_schemes && preg_match('~('.$preg_schemes.'://)(.*)~', $url->url)){
			$api_url = "{$this->config["url"]}/static/globe.png";
			header("Location: $api_url");	
			exit;	
		}

		$api_url = "https://ico.mikelin.cn/{$url->url}";
		header("Location: $api_url");	
		exit;					
	}
