...
class App{
	...
	protected function user(){
		...
		if($this->requireUpgrade()) return Main::redirect(Main::href("pricing","",FALSE));
		// Get URLs
	...
	protected function profile(){
		...
		if(!empty($this->id) && is_numeric($id[0]) && $bundle = $this->db->get("bundle",array("id"=>"?","access"=>"?"),array("limit"=>1),array($id[0],"public"))){
			...
			Main::set("title",$user->username." ".e("Public profile of "));
			Main::set("description","{$user->username}个人书签，{$user->username}个人网址库，{$user->username}导航.");
	...
	*/
	...
	protected function _DNS404(){
		...
		$content ="<section class='blue rounded wshadow'><h1>Yes</h1>
							<h4>Your domain name is working. Add it to your account now.</h4></section>";			
	...
	protected function header(){
		...
		if($this->config["demo"]){
			Main::add("<script type='text/javascript'>$('body').append('<div class=\"run-demo\"><a href=\"https://codecanyon.net/item/premium-url-shortener/3688135?ref=fengge\" class=\"btn btn-secondary\"><i class=\"glyphicon glyphicon-shopping-cart\"></i> <h3>Want to run your own shortener? <span>You can purchase this script right now.</span></h3></div>');</script>","custom");			
      	...
      	if(empty($data)){
        	...
        	$html.="<ul>";
        		foreach ($data as $url) {
        		  ...
        		  &nbsp;<img src='https://ico.mikelin.cn/{$url->url}' alt='favicon'>
