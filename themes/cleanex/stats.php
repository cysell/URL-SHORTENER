				...
				<div class="col-md-9 url-info">
					...
					<div class="row">
						...
						<div class="col-sm-4 text-center">
							<p>
								<i class="glyphicon glyphicon-link"></i> <?php echo (!empty($url->domain) ? $url->domain : $this->config["url"]) ?>/<?php echo $url->alias.$url->custom ?> <a href="#copy" class="copy inline-copy hidden-xs"  data-clipboard-text="<?php echo (!empty($url->domain) ? $url->domain : $this->config["url"]) ?>/<?php echo $url->alias.$url->custom ?>"><?php echo e("Copy")?></a>								
							</p>
							<p>
								<i class="glyphicon glyphicon-qrcode"></i> <?php echo $this->config["url"] ?>/<?php echo $url->alias.$url->custom ?>/qr <a href="#copy" class="copy inline-copy hidden-xs"  data-clipboard-text="<?php echo (!empty($url->domain) ? $url->domain : $this->config["url"]) ?>/<?php echo $url->alias.$url->custom ?>/qr"><?php echo e("Copy")?></a>								
							</p>	
								<div class="btn-group btn-group-sm">
								<a href="https://www.facebook.com/sharer.php?u=<?php echo(!empty($url->domain) ? $url->domain : $this->config["url"]) ?>/<?php echo $url->alias.$url->custom ?>" class="btn btn-facebook u_share"><?php echo e("Share on") ?> Facebook</a>
	            	<a href="https://twitter.com/share?url=<?php echo(!empty($url->domain) ? $url->domain : $this->config["url"]) ?>/<?php echo $url->alias.$url->custom ?>&amp;text=Check+out+this+url" class="btn btn-twitter u_share"><?php echo e("Share on") ?> Twitter</a>								
							</div>
						</div>						
					</div>
				</div>
